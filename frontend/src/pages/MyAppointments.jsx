...
const MyAppointments = () => {
  ...
  const initPay = (order) => {
    const options = {
      key: import.meta.env.VITE_RAZORPAY_KEY_ID, // ✅ Test Mode Key from .env
      amount: order.amount,
      currency: order.currency,
      name: "Test Mode - Appointment Payment",
      description: "Test Payment for Appointment",
      order_id: order.id,
      receipt: order.receipt,
      sandbox: true, // ✅ Explicitly setting sandbox mode for test payments
  ...
  return (
    <div>
      <p className="pb-3 mt-12 font-medium text-zinc-700 border-b">My Booking (Test Mode)</p>
      <div>
          ...
          <div key={index} className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b">
            ...
            <div className="flex flex-col gap-2 justify-end">
              {!item.cancelled && item.payment && !item.isCompleted && (
                <button className="sm:min-w-48 py-2 border rounded text-[#111010] bg-[#008000]">Paid</button>
              )}
